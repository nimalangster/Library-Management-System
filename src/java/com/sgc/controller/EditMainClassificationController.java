...
public class EditMainClassificationController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
          ...
          mainClass.setId(classId);
          mainClass.setName(className);
        
        try{
            
            boolean success = mainClassDao.updateMainClass(mainClass);
            if (success)
            {                   
                request.getRequestDispatcher("SearchMainClassificationController?mode=Edited").forward(request, response);               
            }
        }catch(Exception ex){
